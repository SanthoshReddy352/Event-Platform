 ...
 */
export async function sendApprovalEmail({ to, from, participantName, eventTitle, eventStartDate, eventEndDate, clubLogoUrl }) {
        ...
        <div class="content">
          ...
          ${clubLogoUrl ? `
            <div class="club-logo-container">
              ...
              <p style="margin-top: 5px; font-size: 14px; color: #555;">Hosted by</p>
            </div>
          ` : ''}
 ...
 */
export async function sendRejectionEmail({ to, from, participantName, eventTitle, reason, eventStartDate, eventEndDate, clubLogoUrl }) {
 ...
 */
async function sendEmail({ to, subject, html, from }) { // 'from' is an object { name, email } or undefined
  console.log('=== EMAIL SEND REQUEST ===');
  console.log('To:', to);
  console.log('Subject:', subject);
  
  const defaultFrom = process.env.EMAIL_FROM || 'EventX <noreply@eventx.com>';
  
  // Format the 'from' string for the email service
  let fromString;
  if (from && from.email) {
    // Format as "Name <email@example.com>" if name is provided, else just "email@example.com"
    fromString = from.name ? `${from.name.replace(/"/g, '')} <${from.email}>` : from.email;
  } else {
  ...
  if (supabaseFunctionUrl) {
    try {
      const response = await fetch(supabaseFunctionUrl, {
        ...
        body: JSON.stringify({ to, subject, html, from: fromString }) // Pass the formatted string
      });
  ...
  if (EMAIL_SERVICE_URL && EMAIL_API_KEY) {
    try {
      const response = await fetch(EMAIL_SERVICE_URL, {
        ...
        body: JSON.stringify({ from: fromString, to, subject, html })
      });
